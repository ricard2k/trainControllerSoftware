...
InputPopupPage::InputPopupPage(TFT_eSPI *tft, const String &prompt, InputMode mode,
                               std::function<void(String, bool)> onComplete)
    : tft(tft), prompt(prompt), mode(mode), onComplete(onComplete)
{
  buildKeyboard();
  rows = (keys.size() + cols - 1) / cols;
}

TFT_eSPI *InputPopupPage::getDisplay() const
{
  return tft;
}
...
void InputPopupPage::buildKeyboard()
{
  ...
  keyWidth = (TC_SCREEN_WIDTH - (cols + 1) * keyPadding) / cols;
  rows = (keys.size() + cols - 1) / cols;
  keyHeight = (TC_SCREEN_HEIGHT - startY - keyPadding * (rows + 1)) / rows;
}
...
void InputPopupPage::draw()
{
  ...
  tft->drawCentreString(prompt, TC_SCREEN_WIDTH / 2, 20, 2);
  drawInputBox();
  drawKeyboard();
}
...
{
  int boxWidth = TC_SCREEN_WIDTH - 20;
  int boxHeight = 30;
  int x = 10;
  int y = 60;
...
void InputPopupPage::handleKeyPress()
{
  ...
  int fontHeight = tft->fontHeight(2);
  int textY = y + (keyHeight - fontHeight) / 2;
  tft->drawCentreString(key, x + keyWidth / 2, textY, 2);
  delay(100);
